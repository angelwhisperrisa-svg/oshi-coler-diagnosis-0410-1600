...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .start-visual .orb { width: 150%; height: 150%; border-radius: 50%; background: radial-gradient(circle at 30% 30%, #fde2ff 0%, #f9a8d4 40%, #c084fc 60%,#7dd3fc 100%); filter: brightness(1.1) saturate(1.2); }
  
  .start-visual::after {
    content: '💖'; position: absolute; top: 70%; left: 70%; transform: translate(-50%, -50%);
    font-size: 20px; background: linear-gradient(135deg, #dfdc09, #ecd3ac);
    -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;
    animation: sparkle　s infinite ease-in-out;
  }
  @keyframes sparkle {
    0%, 100% { transform: translate(-50%, -50%) scale(1.3); opacity: 1; }
    50%       { transform: translate(-50%, -50%) scale(1.2); opacity: 1; }
  }
  .start-btn {
    appearance: none; -webkit-appearance: none;
    background: linear-gradient(135deg, #e8b4cc, #d4a8e0,#d4a8e0);
    ...
    box-shadow: 0 6px 20px rgba(200,150,200,0.3);
  }
